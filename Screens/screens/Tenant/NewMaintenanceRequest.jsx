...
const NewMaintenanceRequest = () => {
  ...
  const submitRequest = () => {
    ...
    Alert.alert("Success", "Your maintenance request has been submitted successfully", [
    ...
    <StyledView className="flex-1 bg-[#f8f9fa]">
      ...
      <StyledView className="bg-[#1a2c4e] pt-12 pb-6 px-4">
        <StyledView className="flex-row justify-between items-center">
          ...
          <StyledText className="text-white text-xl font-bold">New Maintenance Request</StyledText>
      ...
      <ScrollView className="flex-1 px-4 pt-6">
        ...
        <StyledView className="bg-white p-4 rounded-xl mb-4 shadow-md">
          <StyledText className="text-[#1a2c4e] font-bold mb-2">Title</StyledText>
          <StyledTextInput
        ...
        {/* Category Selection */}
        <StyledView className="bg-white p-4 rounded-xl mb-4 shadow-md">
          <StyledText className="text-[#1a2c4e] font-bold mb-2">Category</StyledText>
          <StyledView className="flex-row flex-wrap justify-between">
            {categories.map((category) => (
              <StyledTouchableOpacity
                key={category.id}
                className={`w-[31%] p-3 rounded-lg mb-3 items-center ${
                  formData.category === category.id ? "bg-[#27ae60]" : "bg-[#f8f9fa] border border-[#e9ecef]"
                }`}
              ...
              >
                <Ionicons
                  name={category.icon}
                  size={24}
                  color={formData.category === category.id ? "white" : "#8395a7"}
                />
                <StyledText
                  className={`mt-1 text-sm ${formData.category === category.id ? "text-white" : "text-[#1a2c4e]"}`}
                >
                  {category.name}
                </StyledText>
              </StyledTouchableOpacity>
        ...
        {/* Description Input */}
        <StyledView className="bg-white p-4 rounded-xl mb-4 shadow-md">
          <StyledText className="text-[#1a2c4e] font-bold mb-2">Description</StyledText>
          <StyledTextInput
        ...
        {/* Priority Selection */}
        <StyledView className="bg-white p-4 rounded-xl mb-4 shadow-md">
          <StyledText className="text-[#1a2c4e] font-bold mb-2">Priority</StyledText>
          <StyledView className="flex-row justify-between">
            {priorities.map((priority) => (
              ...
              >
                <StyledText className={`font-bold ${formData.priority === priority ? "text-white" : "text-[#1a2c4e]"}`}>
                  {priority}
                </StyledText>
              </StyledTouchableOpacity>
            ))}
        ...
        {/* Image Upload */}
        <StyledView className="bg-white p-4 rounded-xl mb-4 shadow-md">
          <StyledText className="text-[#1a2c4e] font-bold mb-2">Add Photos (Optional)</StyledText>
        ...
        {/* Submit Button */}
        <StyledTouchableOpacity className="bg-[#27ae60] p-4 rounded-xl mb-8 items-center" onPress={submitRequest}>
          <StyledText className="text-white font-bold text-lg">Submit Request</StyledText>
        </StyledTouchableOpacity>
      </ScrollView>
    </StyledView>
  )
